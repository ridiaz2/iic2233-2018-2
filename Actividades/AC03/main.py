...
def codificador(string):
    ...
    """
    lista_1 = "arquetipos0123456789"
    lista_2 = "0123456789arquetipos"
    def reemplazar(caracter):
        if caracter in lista_1:
            return lista_2[lista_1.find(caracter)]
        return caracter
    return "".join(list(map(reemplazar, string)))
...
def pasaprogra(funcion):
    ...
    def wrap(respuesta, palabra_correcta):
        if respuesta.lower() == "pasaprogra":
            return palabra_correcta, 0
        return funcion(respuesta, palabra_correcta)
    return wrap
...
def desencriptar(funcion_decodificadora):
    ...
    def decorador(funcion):
        def wrap(ruta):
            retorno = funcion(ruta)
            for i in retorno
                yield list(map(funcion_decodificadora, retorno))
        yield wrap
    yield decorador
